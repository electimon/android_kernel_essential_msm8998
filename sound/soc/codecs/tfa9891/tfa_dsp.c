...
static void tfa_set_query_info (int dev_idx) {
	...
	case 0x12:
		...
		break;
	default:
		pr_err("tfa98xx: %s() unknown device type : 0x%02x\n", __func__, handles_local[dev_idx].rev);
		_ASSERT(0);
		break;
	}
}
...
// TODO: how do we deal with old bugs?
		...
		if (Tfa98xx_Error_Ok != error) {
			...
			pr_err("\ntfa98xx: %s() Error: Unable to read revid from slave:0x%02x \n", __func__, slave_address/2);
			return error;
		}
		...
		*pHandle = idx;
		...
		pr_err("tfa98xx: %s() slave:0x%02x revid:0x%04x\n", __func__, slave_address, rev);
#endif
		break;
	default:
		/* wrong slave address */
...
void tfa98xx_apply_deferred_calibration(Tfa98xx_handle_t handle)
{
	...
	if (controls->otc.deferrable && controls->otc.triggered) {
		pr_err("tfa98xx: %s() Deferred writing otc = %d\n", __func__, controls->otc.wr_value);
		...
		if (err != Tfa98xx_Error_Ok) {
			pr_err("tfa98xx: %s() Unable to apply deferred MTP OTC write. Error=%d\n", __func__,
									err);
		} else {
	...
	if (controls->mtpex.deferrable && controls->mtpex.triggered) {
		pr_err("tfa98xx: %s() Deferred writing mtpex = %d\n", __func__, controls->mtpex.wr_value);
		err = tfa98xx_set_mtp(handle,
			(uint16_t)controls->mtpex.wr_value << TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_POS,
			1 << TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_POS);
		if (err != Tfa98xx_Error_Ok) {
			pr_err("tfa98xx: %s() Unable to apply deferred MTPEX write. Rrror=%d\n", __func__,
									err);
		} else {
	...
	if (controls->calib.triggered) {
		...
		if(err) {
			pr_info("tfa98xx: %s() Deferred calibration failed: %d\n", __func__, err);
		} else {
			pr_err("tfa98xx: %s() Deferred calibration ok\n", __func__);
...
enum Tfa98xx_Error tfa98xx_get_mtp(Tfa98xx_handle_t handle, uint16_t *value)
{
	...
	if ( TFA_GET_BF(handle, PWDN) ) {
		pr_err("tfa98xx: %s() PLL in powerdown\n", __func__);
	...
	if (tfa98xx_dev_family(handle) == 1) {
		tfa98xx_dsp_system_stable(handle, &status);
		if (status==0) {
			pr_err("tfa98xx: %s() PLL not running\n", __func__);
			return Tfa98xx_Error_NoClock;
		}
	}
...
void tfa98xx_set_exttemp(Tfa98xx_handle_t handle, short ext_temp)
{
	if ((-256 <= ext_temp) && (ext_temp <= 255)) {
		...
		pr_err("tfa98xx: %s() Using ext temp %d C\n", __func__, twos(ext_temp));
		TFA_SET_BF(handle, TROS, 1);
		TFA_SET_BF(handle, EXTTS, twos(ext_temp));
	} else {
		pr_err("tfa98xx: %s() Clearing ext temp settings\n", __func__);
...
{
	if (!tfa98xx_handle_is_open(handle)) {
		pr_err("tfa98xx: %s() device not opened\n", __func__);
		return Tfa98xx_Error_NotOpen;
	}
...
tfa98xx_check_ic_rom_version(Tfa98xx_handle_t handle, const unsigned char patchheader[])
{
	...
	if (checkaddress != 0xFFFF) {
		...
		if (error == Tfa98xx_Error_Ok) {
			if (value != checkvalue) {
				pr_err("tfa98xx: %s() patch file romid type check failed [0x%04x]: expected 0x%02x, actual 0x%02x\n", __func__,
		...
		if ( checkvalue != 0xFFFFFF && checkvalue != 0) {
			...
			if ( revid != handles_local[handle].rev) {
				pr_err("tfa98xx: %s() patch file device type check failed: expected 0x%02x, actual 0x%02x\n", __func__,
...
int tfa_set_bf(Tfa98xx_handle_t dev_idx, const uint16_t bf, const uint16_t value)
{
	...
	if (err) {
              pr_err("tfa98xx: %s() Error getting bf :%d \n", __func__, -err);
              return -err;
        }
	...
	regvalue |= value<<pos;
        ...
        if(oldvalue != regvalue) {
	        ...
	        if (err) {
                      pr_err("tfa98xx: %s() Error setting bf :%d \n", __func__, -err);
                      return -err;
                }
        }
...
enum Tfa98xx_Error tfa_cf_powerup(Tfa98xx_handle_t handle) {
	...
	if (tfa98xx_runtime_verbose)
		pr_info("tfa98xx: %s() Waiting for DSP system stable...\n", __func__);
	...
	if (tries==0) {// timedout
		pr_err("tfa98xx: %s() DSP subsystem start timed out\n", __func__);
		return Tfa98xx_Error_StateTimedOut;
	}
...
static enum Tfa98xx_Error tfa98xx_aec_output(Tfa98xx_handle_t handle, int enable)
{
	...
	if (tfa98xx_dev_family(handle) == 1)
		...
		pr_err("tfa98xx: %s() I2SDOE on unsupported family\n", __func__);
...
/*
 * set MTPOTC to one-time calibration
 */
enum Tfa98xx_Error tfa98xxSetCalibrateOnce(Tfa98xx_handle_t handle)
{
	...
	if (err != Tfa98xx_Error_Ok) {
		pr_err("tfa98xx: %s() set MTP register fail, err=%d\n", __func__, err);
    }
...
enum Tfa98xx_Error show_current_state(Tfa98xx_handle_t handle)
{
	...
	pr_err("tfa98xx: %s() Current HW manager state: ", __func__);

	switch(manstate) {
		case 0: pr_err("tfa98xx: %s() power_down_state \n", __func__);
			break;
		case 1: pr_err("tfa98xx: %s() wait_for_source_settings_state \n", __func__);
			break;
		case 2: pr_err("tfa98xx: %s() connnect_pll_input_state \n", __func__);
			break;
		case 3: pr_err("tfa98xx: %s() disconnect_pll_input_state \n", __func__);
			break;
		case 4: pr_err("tfa98xx: %s() enable_pll_state \n", __func__);
			break;
		case 5: pr_err("tfa98xx: %s() enable_cgu_state \n", __func__);
			break;
		case 6: pr_err("tfa98xx: %s() init_cf_state \n", __func__);
			break;
		case 7: pr_err("tfa98xx: %s() enable_amplifier_state \n", __func__);
			break;
		case 8: pr_err("tfa98xx: %s() alarm_state \n", __func__);
			break;
		case 9: pr_err("tfa98xx: %s() operating_state \n", __func__);
			break;
		case 10: pr_err("tfa98xx: %s() mute_audio_state \n", __func__);
			break;
		case 11: pr_err("tfa98xx: %s() disable_cgu_pll_state \n", __func__);
			break;
		default:
			pr_err("tfa98xx: %s() Unable to find current state \n", __func__);
			break;
	}
...
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	pr_err("tfa98xx: %s() coldstart%s :", __func__, force? " (forced)":"");
...
#define CF_CONTROL 0x8100
	...
	while ( state != TFA_GET_BF(handle, ACS)) {
		...
		if (tries-- == 0) {
			pr_err("tfa98xx: %s() coldboot (ACS) did not %s\n", __func__, state ? "set":"clear");
...
enum Tfa98xx_Error tfaRunStartup(Tfa98xx_handle_t handle, int profile)
{
	...
	if(!noinit) {
		...
		pr_err("\ntfa98xx: %s() Warning: No init keyword found in the cnt file. Init is skipped! \n", __func__);
	}
	...
	if (tfa98xx_runtime_verbose) {
		if (TFA_GET_BF(handle, NOCLK))
			pr_err("tfa98xx: %s() Using internal clock\n", __func__);
		pr_err("tfa98xx: %s() Waiting for DSP system stable...\n", __func__);
	}
	...
	if (tries == CFSTABLE_TRIES) {
		if (tfa98xx_runtime_verbose) pr_err("tfa98xx: %s() Timed out\n", __func__);
		return Tfa98xx_Error_StateTimedOut;
	}  else
		if (tfa98xx_runtime_verbose) pr_err("tfa98xx: %s()  OK (tries=%d)\n", __func__, tries);
...
enum Tfa98xx_Error tfaRunWaitCalibration(Tfa98xx_handle_t handle, int *calibrateDone)
{
	...
	*calibrateDone = 0;
	...
	if(*calibrateDone != 1) {
		pr_err("tfa98xx: %s() Calibration failed! \n", __func__);
		err = Tfa98xx_Error_Bad_Parameter;
	} else if (tries==TFA98XX_API_WAITRESULT_NTRIES) {
		pr_err("tfa98xx: %s() Calibration has timedout! \n", __func__);
		err = Tfa98xx_Error_StateTimedOut;
	} else if(tries_mtp_busy == 1000) {
		pr_err("tfa98xx: %s() Calibrate Failed: MTP_busy stays high! \n", __func__);
		err = Tfa98xx_Error_StateTimedOut;
	}
...
enum Tfa98xx_Error tfa_start(int next_profile, int *vstep)
{
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if (tfa98xx_runtime_verbose) {
			pr_err("tfa98xx: %s() active_profile:%s, next_profile:%s\n", __func__,
					tfaContProfileName(dev,active_profile ),
					tfaContProfileName(dev, next_profile));
			pr_err("tfa98xx: %s() Starting device [%s]\n", __func__, tfaContDeviceName(dev));
		...
		if (cal_profile >= 0) {
			next_profile = 0;
			pr_err("tfa98xx: %s() Loading %s profile! \n", __func__, tfaContProfileName(dev, next_profile));
		}
	}
...
enum Tfa98xx_Error tfa_stop(void)
{
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if ( err != Tfa98xx_Error_Ok)
			goto error_exit;
		if (tfa98xx_runtime_verbose)
			pr_err("tfa98xx: %s() Stopping device [%s]\n", __func__, tfaContDeviceName(dev));
		/* mute */
		tfaRunMute(dev);
		/* powerdown CF */
		err = tfa98xx_powerdown(dev, 1 );
...
enum Tfa98xx_Error tfa_reset(void)
{
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if ( err != Tfa98xx_Error_Ok)
			break;
		if (tfa98xx_runtime_verbose)
			pr_err("tfa98xx: %s() resetting device [%s]\n", __func__, tfaContDeviceName(dev));
...
tfa_dsp_get_calibration_impedance(Tfa98xx_handle_t handle)
{
	...
	if (error == Tfa98xx_Error_Ok) {
		...
		if (TFA_GET_BF(handle, MTPOTC) && ((handles_local[handle].rev & 0xff) == 0x88)) {
			if (tfa98xx_runtime_verbose)
				pr_err("tfa98xx: %s() Getting calibration values from MTP\n", __func__);
			...
			if (tfa98xx_runtime_verbose)
				pr_err("tfa98xx: %s() Getting calibration values from Speakerboost\n", __func__);
