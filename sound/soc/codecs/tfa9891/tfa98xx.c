...
static int tfa98xx_input_open(struct input_dev *dev)
{
	...
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		dev_err(&tfa98xx->i2c->dev,
			"tfa98xx: DSP not loaded, cannot start tap-detection\n");
		return -EIO;
	}
...
static int tfa98xx_register_inputdev(struct tfa98xx *tfa98xx)
{
	...
	if (!input) {
		dev_err(tfa98xx->codec->dev, "tfa98xx: Unable to allocate input device\n");
		return -ENOMEM;
	}
	...
	if (err) {
		dev_err(tfa98xx->codec->dev, "tfa98xx: Unable to register input device\n");
		goto err_free_dev;
	}
	...
	for (i = 0; i < tfaContMaxProfile(tfa98xx->handle); i++) {
		if (strstr(tfaContProfileName(tfa98xx->handle, i), ".tap")) {
			...
			dev_dbg(tfa98xx->codec->dev,
				"tfa98xx: found a tap-detection profile (%d - %s)\n",
...
static int tfa98xx_dbgfs_otc_get(void *data, u64 *val)
{
	...
	if (otc->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			pr_err("tfa98xx: Unable to check DSP access: %d\n", err);
			return -EIO;
		} else if (err == Tfa98xx_Error_NoClock) {
			if (otc->rd_valid) {
				/* read cached value */
				*val = otc->rd_value;
				pr_err("tfa98xx: Returning cached value of OTC: %llu\n", *val);
			} else {
				pr_err("tfa98xx: OTC value never read!\n");
				return -EIO;
			}
			return 0;
		}
...
static int tfa98xx_dbgfs_otc_set(void *data, u64 val)
{
	...
	if (val != 0 && val != 1) {
		pr_err("tfa98xx: Unexpected value %llu\n\n", val);
		return -EINVAL;
	}
	mutex_lock(&tfa98xx->dsp_lock);
	status = tfa98xx_open(tfa98xx->handle);
	...
	if (otc->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			otc->wr_value = val;
			otc->triggered = true;
			pr_err("tfa98xx: Deferring write to OTC (%d)\n", otc->wr_value);
			return 0;
		}
	}
...
static int tfa98xx_dbgfs_mtpex_set(void *data, u64 val)
{
	...
	if (val != 0) {
		pr_err("tfa98xx: Can only clear MTPEX (0 value expected)\n");
		return -EINVAL;
	}
	...
	if (mtpex->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			pr_err("tfa98xx: Deferring write to MTPEX (%d)\n", mtpex->wr_value);
			return 0;
		}
	}

	pr_err("tfa98xx: mtpex < 0\n");
...
void tfa98xx_deferred_calibration_status(Tfa98xx_handle_t handle, int calibrateDone)
{
	...
	if (calib->wr_value) {
		...
		case 1:
			...
			break;
		case 0:
			pr_err("tfa98xx: Calibration not complete, still waiting...\n");
			break;
		case -1:
			pr_err("tfa98xx: Calibration failed\n");
			calib->wr_value = false; /* calibration over */
			calib->rd_valid = true;  /* result available */
			...
			break;
		default:
			pr_err("tfa98xx: Unknown calibration status: %d\n",
							calibrateDone);
		}
	}
}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if(ret) {
		pr_err("tfa98xx: Calibration start failed (%d), deferring...\n", ret);
		calib->triggered = true;
	} else {
		pr_err("tfa98xx: Calibration started\n");
	}
				     ...
				     loff_t *ppos)
{
	...
	pr_err("tfa98xx: calib_done: %d - ret = %d - %s", calibrate_done, ret, str);
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, start_cmd, sizeof(start_cmd) - 1)) {
		pr_err("tfa98xx: Manual triggering of dsp start...\n");
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa98xx_tfa_start(tfa98xx, tfa98xx_profile, tfa98xx_vsteps);
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_err("tfa98xx: tfa_start complete: %d\n", ret);
	} else if (!strncmp(buf, stop_cmd, sizeof(stop_cmd) - 1)) {
		pr_err("tfa98xx: Manual triggering of dsp stop...\n");
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa_stop();
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_err("tfa98xx: tfa_stop complete: %d\n", ret);
	} else if (!strncmp(buf, mon_start_cmd, sizeof(mon_start_cmd) - 1)) {
		pr_err("tfa98xx: Manual start of monitor thread...\n");
		queue_delayed_work(tfa98xx->tfa98xx_wq,
					&tfa98xx->monitor_work, HZ);
	} else if (!strncmp(buf, mon_stop_cmd, sizeof(mon_stop_cmd) - 1)) {
		pr_err("tfa98xx: Manual stop of monitor thread...\n");
...
#define TFA98XX_DEBUGFS_REG_CREATE_FILE(__reg, __name)				\
	debugfs_create_file(TOSTRING(__reg) "-" TOSTRING(__name), 0660, dbg_reg_dir,\
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("OTC", 0660, tfa98xx->dbg_dir,
						i2c, &tfa98xx_dbgfs_calib_otc_fops);
	debugfs_create_file("MTPEX", 0660, tfa98xx->dbg_dir,
						i2c, &tfa98xx_dbgfs_calib_mtpex_fops);
	debugfs_create_file("TEMP", 0660, tfa98xx->dbg_dir,
						i2c, &tfa98xx_dbgfs_calib_temp_fops);
	debugfs_create_file("calibrate", 0660, tfa98xx->dbg_dir,
	...
	debugfs_create_file("dsp-state", 0660, tfa98xx->dbg_dir,
	...
	if (tfa98xx->flags & TFA98XX_FLAG_SAAM_AVAILABLE) {
		dev_err(tfa98xx->dev, "tfa98xx: Adding pga_gain debug interface\n");
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (profile == tfa98xx_profile) {
		...
		if (ready) {
			...
			if (err) {
				pr_err("tfa98xx: Write vstep error: %d\n", err);
			} else {
				pr_err("tfa98xx: Succesfully changed vstep index!\n");
			}
		}
	...
	pr_err("tfa98xx: vstep:%d, (control value: %d) - profile %d\n", new_vstep,
								 value, profile);
	return (err == 0);
}
		       ...
		       struct snd_ctl_elem_info *uinfo)
{
	...
	pr_debug("tfa98xx: vsteps count: %d [prof=%d]\n", tfacont_get_max_vstep(0, profile),
			profile);
	return 0;
}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_err("tfa98xx: selected container profile [%d]\n", prof_idx);
	...
	if (ready) {
		...
		if (err) {
			pr_err("tfa98xx: Write profile error: %d\n", err);
		} else {
			pr_err("tfa98xx: Changed to profile %d (vstep = %d)\n", prof_idx,
							tfa98xx_vsteps[0]);
		}
	}
...
static int add_sr_to_profile(struct tfa98xx *tfa98xx, char *basename, int len, int profile)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if (0 == strncmp(bprof->basename, basename, len)) {
			...
			if (!sr) {
				pr_err("tfa98xx: unable to identify supported sample rate for %s\n", bprof->basename);
				return -1;
			}
			...
			if (idx < 0 || idx >= TFA98XX_NUM_RATES) {
				pr_err("tfa98xx: invalid index for samplerate %d\n", idx);
				return -1;
			}
			...
			pr_err("tfa98xx: added profile:samplerate = [%d:%d] for mixer profile: %s\n", profile, sr, bprof->basename);
		}
	}
...
reg_err:
	dev_err(&tfa98xx->i2c->dev,
		"tfa98xx: Unable to match I2C address 0x%x with a container device\n",
							tfa98xx->i2c->addr);
	return -EINVAL;
}
...
static void tfa98xx_unregister_dsp(struct tfa98xx *tfa98xx)
{
	...
	dev_info(&tfa98xx->i2c->dev, "tfa98xx: Un-registered DSP instance with handle %d\n",
							tfa98xx->handle);
}
					...
					unsigned short value)
{
	...
	if (tfa98xx_devices[handle]) {
		...
		if (!tfa98xx || !tfa98xx->regmap) {
			pr_err("tfa98xx: No tfa98xx regmap available\n");
			return Tfa98xx_Error_Bad_Parameter;
		}
retry:
		ret = regmap_write(tfa98xx->regmap, subaddress, value);
		if (ret < 0) {
			pr_err("tfa98xx: i2c error, retries left: %d\n", retries);
			if (retries) {
				retries--;
		...
		if (tfa98xx_kmsg_regs)
			dev_err(&tfa98xx->i2c->dev, "tfa98xx:   WR reg=0x%02x, val=0x%04x %s\n",
								subaddress, value,
								ret<0? "Error!!" : "");
		...
		pr_err("tfa98xx: No device available\n");
...
retry:
		...
		if (ret < 0) {
			pr_err("tfa98xx: i2c error at subaddress 0x%x, retries left: %d\n", subaddress, retries);
			if (retries) {
				retries--;
				msleep(I2C_RETRY_DELAY);
				goto retry;
		...
		*val = value & 0xffff;
		...
		if (tfa98xx_kmsg_regs)
			dev_err(&tfa98xx->i2c->dev, "tfa98xx: RD   reg=0x%02x, val=0x%04x %s\n",
								subaddress, *val,
								ret<0? "Error!!" : "");
		if (tfa98xx_ftrace_regs)
				...
				int len, unsigned char value[])
{
	...
	if (tfa98xx_devices[handle] && tfa98xx_devices[handle]->i2c) {
		...
		if (err != ARRAY_SIZE(msgs)) {
			dev_err(&tfa98xx_client->dev, "tfa98xx: read transfer error %d\n",
									err);
			error = Tfa98xx_Error_Fail;
		}

		if (tfa98xx_kmsg_regs)
			dev_err(&tfa98xx_client->dev, "tfa98xx: RD-DAT reg=0x%02x, len=%d\n",
								reg, len);
		if (tfa98xx_ftrace_regs)
...
retry:
		...
		if (ret == len) {
			if (tfa98xx_kmsg_regs)
				dev_err(&tfa98xx->i2c->dev, "tfa98xx:   WR-RAW len=%d\n", len);
			if (tfa98xx_ftrace_regs)
				tfa98xx_trace_printk("\t\tWR-RAW len=%d\n", len);
			return Tfa98xx_Error_Ok;
		}
		pr_err("tfa98xx:   WR-RAW (len=%d) Error I2C send size mismatch %d\n", len, ret);
		error = Tfa98xx_Error_Fail;
	} else {
		pr_err("tfa98xx: No device available\n");
		error = Tfa98xx_Error_Fail;
							...
							unsigned int mask)
{
	...
	if (reg & errs & ~mask) {
		...
		dev_dbg(tfa98xx->codec->dev, "tfa98xx: status_reg events: 0x%x\n", reg);
	}
...
static void tfa98xx_tapdet_check_update(struct tfa98xx *tfa98xx)
{
	...
	if (!tfa98xx->tapdet_enabled && enable) {
		...
		err = tfa98xx_dsp_write_mem_word(tfa98xx->handle, XMEM_TAP_ACK, 0,
							Tfa98xx_DMEM_XMEM);
		if (err)
			pr_err("tfa98xx: Unable to write to XMEM\n");
		...
		if (count > 1)
			pr_err("tfa98xx: Had to run %d times to ack SPKS at init\n", count);
	}
...
static void __tfa98xx_interrupt_setup(struct tfa98xx *tfa98xx)
{
	...
	dev_info(&tfa98xx->i2c->dev, "tfa98xx: Initial interrupts setup: ICR = 0x%04x\n", ie_reg);
}
...
static void __tfa98xx_interrupt_restore(struct tfa98xx *tfa98xx)
{
	...
	if (ie_reg != handles_local[tfa98xx->handle].interrupt_enable[0]) {
		...
		dev_info(&tfa98xx->i2c->dev, "tfa98xx: Restored interrupts: ICR = 0x%04x\n",
									ie_reg);
	} else {
		dev_info(&tfa98xx->i2c->dev, "tfa98xx: No interrupt restore needed\n");
	}
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (!container) {
		...
		pr_err("tfa98xx: Error allocating memory\n");
		return;
	}
	...
	pr_err("tfa98xx: %.2s%.2s\n", container->version, container->subversion);
	pr_err("tfa98xx: %.8s\n", container->customer);
	pr_err("tfa98xx: %.8s\n", container->application);
	pr_err("tfa98xx: %.8s\n", container->type);
	pr_err("tfa98xx: %d ndev\n", container->ndev);
	pr_err("tfa98xx: %d nprof\n", container->nprof);
	...
	if (tfa98xx->handle < 0) {
		dev_err(tfa98xx->dev, "tfa98xx: Cannot register with DSP, aborting\n");
		return;
	}

	if (tfa_probe(tfa98xx->i2c->addr << 1, &handle) != Tfa98xx_Error_Ok) {
		dev_err(tfa98xx->dev, "tfa98xx: Failed to probe TFA98xx @ 0x%.2x\n", tfa98xx->i2c->addr);
		return;
	}
	...
	if (strcmp(dflt_prof_name, "")) {
		...
		if (i >= tfaContMaxProfile(tfa98xx->handle))
			dev_err(tfa98xx->dev,
				"tfa98xx: Default profile override failed (%s profile not found)\n",
				dflt_prof_name);
	}


	tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_OK;
	pr_err("tfa98xx: Firmware init complete\n");
...
static void tfa98xx_tapdet(struct tfa98xx *tfa98xx)
{
	...
	switch (mem) {
	case 0xffffffff:
		pr_err("tfa98xx: More than 4 taps detected! (flagTapPattern = -1)\n");
		btn = BTN_0;
		break;
	case 0xfffffffe:
		pr_err("tfa98xx: Single tap detected! (flagTapPattern = -2)\n");
		btn = BTN_0;
		break;
	case 0:
		pr_err("tfa98xx: Unrecognized pattern! (flagTapPattern = 0)\n");
		btn = BTN_0;
		break;
	default:
		pr_err("tfa98xx: Detected pattern: %d\n", mem);
		btn = BTN_0 + mem;
		break;
	}
	...
	if (count > 1)
		pr_err("tfa98xx: Had to run %d times to ack SPKS\n", count);
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if (!tfa98xx->input) {
		 ...
		 */
		val = snd_soc_read(tfa98xx->codec, TFA98XX_STATUSREG);
		pr_debug("tfa98xx: SYS_STATUS0: 0x%04x\n", val);
		if ((TFA98XX_STATUSREG_ACS & val) ||
		   ...
		   !(TFA98XX_STATUSREG_SWS & val)) {
			...
			if (TFA98XX_STATUSREG_ACS & val)
				pr_err("tfa98xx: ERROR: ACS\n");
			if (TFA98XX_STATUSREG_WDS & val)
				pr_err("tfa98xx: ERROR: WDS\n");
			if (TFA98XX_STATUSREG_SPKS & val)
				pr_err("tfa98xx: ERROR: SPKS\n");
			if (!(TFA98XX_STATUSREG_SWS & val))
				pr_err("tfa98xx: ERROR: AMP_SWS\n");
		...
		if (	!(val & TFA98XX_STATUSREG_CLKS) ||
			...
			pr_err("tfa98xx: Misc errors detected: STATUS_FLAG0 = 0x%x\n", val);
		...
		if ((val & TFA98XX_STATUS_FLAGS1_TDMERR) |
			...
			pr_err("tfa98xx: TDM related errors: STATUS_FLAG1 = 0x%x\n", val);
	}
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		pr_err("tfa98xx: Skipping tfa_start (no FW: %d)\n", tfa98xx->dsp_fw_state);
		return;
	}

	if(tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
		pr_err("tfa98xx: Stream already started, skipping DSP power-on\n");
		return;
	}
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		if (ret != Tfa98xx_Error_Ok) {
			 ...
			 */
			dev_err(&tfa98xx->i2c->dev,
					"tfa98xx: tfa_start failed! (err %d) - %d\n",
...
static void tfa98xx_interrupt(struct work_struct *work)
{
	...
	pr_err("tfa98xx: tfa98xx_interrupt\n");
	...
	if (out2) {
		...
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTMWSRC, out2) > 0) {
			...
			if (manwait1 > 0) {
				pr_err("tfa98xx: entering wait for source state\n");
				tfa98xx->count_wait_for_source_state++;

				/* set AMPC and AMPE to make sure the amp is enabled */
				pr_err("tfa98xx: setting AMPC and AMPE to 1 (default) \n");
				...
				pr_err("tfa98xx: leaving wait for source state\n");
						...
						struct snd_soc_dai *dai)
{
	...
	if (NULL == substream->runtime) {
        pr_err("tfa98xx: %s() runtime is NULL. I think it is offload now.\n", __func__);
        return 0;
    }
	...
	for (prof = 0; prof < nprof; prof++) {
		if (0 == strncmp(basename, tfaContProfileName(tfa98xx->handle, prof), len)) {
			...
			if (!sr)
				dev_err(codec->dev, "tfa98xx: Unable to identify supported sample rate\n");
...
static int tfa98xx_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	pr_err("tfa98xx: fmt=0x%x\n", fmt);

	/* Supported mode: regular I2S, slave, or PDM */
	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_I2S:
		if ((fmt & SND_SOC_DAIFMT_MASTER_MASK) != SND_SOC_DAIFMT_CBS_CFS) {
			dev_err(codec->dev, "tfa98xx: Invalid Codec master mode\n");
			return -EINVAL;
		}
		break;
	case SND_SOC_DAIFMT_PDM:
		break;
	default:
		dev_err(codec->dev, "tfa98xx: Unsupported DAI format %d\n",
			     ...
			     struct snd_soc_dai *dai)
{
	...
	pr_debug("tfa98xx: Requested rate: %d, sample size: %d, physical size: %d\n",
	...
	pr_debug("tfa98xx: mixer profile:container profile = [%d:%d]\n", tfa98xx_mixer_profile, prof_idx);

	if (params_channels(params) > 2) {
		pr_err("tfa98xx: Unusual number of channels: %d\n", params_channels(params));
	}
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (no_start) {
		pr_err("tfa98xx: no_start parameter set no tfa_start or tfa_stop, returning\n");
		return 0;
	}
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	pr_err("tfa98xx: Container loading requested: %d\n", ret);

 #if LINUX_VERSION_CODE < KERNEL_VERSION(3,16,0)
	codec->control_data = tfa98xx->regmap;
	ret = snd_soc_codec_set_cache_io(codec, 8, 16, SND_SOC_REGMAP);
	if (ret != 0) {
		dev_err(codec->dev, "tfa98xx: Failed to set cache I/O: %d\n", ret);
		return ret;
	}
#endif
	tfa98xx_add_widgets(tfa98xx);

	dev_info(codec->dev, "tfa98xx: tfa98xx codec registered (%s)",
							tfa98xx->fw.name);
...
static void tfa98xx_irq_9888(struct tfa98xx *tfa98xx)
{
	...
	pr_err("tfa98xx: tfa98xx irq 9888\n");
	...
	pr_err("tfa98xx: interrupt1: 0x%.4x (enabled: 0x%.4x)\n", out1, en1);
	pr_err("tfa98xx: interrupt2: 0x%.4x (enabled: 0x%.4x)\n", out2, en2);
	pr_err("tfa98xx: interrupt3: 0x%.4x (enabled: 0x%.4x)\n", out3, en3);
...
static void __tfa98xx_irq(struct tfa98xx *tfa98xx)
{
	...
	if (!(val & ie)) {
		...
		dev_err(&tfa98xx->i2c->dev, "tfa98xx: ICR: 0x%04x\n", ireg);
	}
#endif
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	if (tfa98xx->reset_gpio < 0)
		dev_err(dev, "tfa98xx: No reset GPIO provided, will not HW reset device\n");

	tfa98xx->irq_gpio =  of_get_named_gpio(np, "irq-gpio", 0);
	if (tfa98xx->irq_gpio < 0)
		dev_err(dev, "tfa98xx: No IRQ GPIO provided.\n");
				...
				char *buf, loff_t off, size_t count)
{
	...
	if (count != 1) {
		pr_err("tfa98xx: invalid register address");
		return -EINVAL;
	}
				...
				char *buf, loff_t off, size_t count)
{
	...
	if (data == NULL) {
		pr_err("tfa98xx: can not allocate memory\n");
		return  -ENOMEM;
	}
			     ...
			     const struct i2c_device_id *id)
{
	...
	pr_err("tfa98xx: %s() enter\n", __func__);

	if (!i2c_check_functionality(i2c->adapter, I2C_FUNC_I2C)) {
		dev_err(&i2c->dev, "tfa98xx: check_functionality failed\n");
		return -EIO;
	}
	...
	if (IS_ERR(tfa98xx->regmap)) {
		...
		dev_err(&i2c->dev, "tfa98xx: Failed to allocate register map: %d\n",
			ret);
		goto err;
	}
	...
	if (np) {
		...
		if (ret) {
			dev_err(&i2c->dev, "tfa98xx: Failed to parse DT node\n");
			goto err;
		}
		if (no_start)
			tfa98xx->irq_gpio = -1;
	...
	if (no_start == 0) {
		...
		case 0x80: /* tfa9890 */
		case 0x81: /* tfa9890 */
			pr_err("tfa98xx: TFA9890 detected\n");
	...
	if (ret < 0) {
		dev_err(&i2c->dev, "tfa98xx: Failed to register TFA98xx: %d\n", ret);
		goto err_off;
	}
	...
	if (gpio_is_valid(tfa98xx->irq_gpio) &&
		...
		dev_err(&i2c->dev, "tfa98xx: Skipping IRQ registration\n");
	...
	if (ret)
		dev_err(&i2c->dev, "tfa98xx: error creating sysfs files\n");
	ret = device_create_bin_file(&i2c->dev, &dev_attr_reg);
	if (ret)
		dev_err(&i2c->dev, "tfa98xx: error creating sysfs files\n");

	pr_err("tfa98xx: %s Probe completed successfully!\n", __func__);
...
static int tfa98xx_i2c_remove(struct i2c_client *i2c)
{
	...
	pr_err("tfa98xx: tfa98xx i2c remove\n");
...
static int __init tfa98xx_i2c_init(void)
{
	...
	pr_err("tfa98xx: TFA98XX driver version %s\n", TFA98XX_VERSION);
